...
public class Main {
    ...
    public static LinkedList<color> allColors() {
        ...
        rand.setSeed(3212341);
        for (int r = 0; r < 256; r++) {
            for (int g = 0; g < 256; g++) {
                for (int b = 0; b < 256; b++) {
                    if(rand.nextInt() % 2 == 0)
                        colors.addFirst(new color(r, g, b));
                    else
                        colors.addLast(new color(r, g, b));
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        LinkedList<Point> orig = listFromImg("lemur2.png");
        ...
        try {
            ImageIO.write(img, "PNG", new File("lemur3"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
